	...
	func _init(line: String) -> void:
		...
		if not scene.begins_with("uid://"):
			...
			if scene.begins_with(":"):
				...
				var resolved_path := scene
				if not resolved_path.begins_with("res://"):
					resolved_path = MetSys.settings._legacy_map_root.path_join(scene)

				if OS.has_feature("editor"):
					var uid := ResourceUID.path_to_uid(resolved_path)
					if not uid.is_empty():
						scene = uid
					else:
						scene = resolved_path
...
func get_room_from_scene_path(scene: String, safe := true) -> String:
	if scene.begins_with("res://") and OS.has_feature("editor"):
		var uid := ResourceUID.path_to_uid(scene)
		if not uid.is_empty():
			scene = uid
